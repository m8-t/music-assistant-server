...
class PocketCastsClient:
    ...
    async def get_episode_transcripts(self, podcast_uuid: str) -> dict[str, list[dict[str, Any]]]:
        """
        Return the available transcripts, keyed by episode UUID, for a podcast.

        Episodes without a transcript are left out.
        ...
        # `transcripts` come from the publisher's podcast:transcript feed tag and
        # `pocket_casts_transcripts` are generated by Pocket Casts, with only one ever filled.
        # The listing endpoint's has_generated_transcript covers only the latter, so it cannot
        # replace this call. Show notes and artwork are dropped to keep the cached entry small.
        episodes: list[dict[str, Any]] = data.get("podcast", {}).get("episodes", [])
        return {
            uuid: transcripts
            for episode in episodes
            if (uuid := episode.get("uuid"))
            and (
                transcripts := episode.get("transcripts") or episode.get("pocket_casts_transcripts")
            )
        }
